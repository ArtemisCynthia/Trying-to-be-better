...
document.addEventListener('touchend', function (event) {
    ...
    if (
        Math.abs(deltax) < 0.3 * document_width && Math.abs(deltay) < 0.3 * document_width
    ) {
        return;
    }
    if ($('#score').text() == success_string) {
...
function move_up() {
    ...
    for (var j = 0; j < 4; j++) {
        for (var i = 1; i < 4; i++) {
            if (board[i][j] != 0) {
                for (var k = 0; k < i; k++) {
                    if (board[k][j] == 0 && no_block_horizontal(j, k, i, board)) {
                        ...
                        board[k][j] == board[i][j] &&
                        no_block_horizontal(j, k, i, board) &&
                        !has_conflicted[k][j]
                    ) {
                        show_move_animation(i, j, k, j);
                        board[k][j] += board[i][j];
...
function move_down() {
    ...
    for (var j = 0; j < 4; j++) {
        for (var i = 2; i >= 0; i--) {
            if (board[i][j] != 0) {
                for (var k = 3; k > i; k--) {
                    if (board[k][j] == 0 && no_block_horizontal(j, i, k, board)) {
                        show_move_animation(i, j, k, j);
                        board[k][j] = board[i][j];
                        board[i][j] = 0;
                        break;
                        ...
                        board[k][j] == board[i][j] &&
                        no_block_horizontal(j, i, k, board) &&
                        !has_conflicted[k][j]
                    ) {
                        show_move_animation(i, j, k, j);
                        board[k][j] += board[i][j];
